      ...
      <div className="flex items-stretch gap-4">
        ...
        <div className="relative bg-green-50 border border-green-100 rounded-2xl px-5 py-4 w-64 overflow-hidden flex flex-col justify-between">
          ...
          <div className="flex items-start gap-3 mb-3">
            ...
            <div>
              <p className="text-sm font-bold text-gray-800">Tạo chuyến đi mới</p>
              <p className="text-xs text-gray-500 mt-0.5 leading-snug">Lên kế hoạch và quản lý chuyến đi của bạn.</p>
            </div>
          </div>

          <button
            onClick={onCreateNew}
            className="w-full bg-green-500 hover:bg-green-600 text-white text-sm font-semibold py-2.5 rounded-xl transition-colors flex items-center justify-center gap-2"
          >
...
export default function TripPage() {
  ...
  const { pause: pauseGps, resume: resumeGps, stopSharing } = useGpsTracking(trips.map((t) => t.id));
            ...
            {!selectedTrip ? <NoTripState onCreateNew={() => setShowCreate(true)} /> : (
              <>
                ...
                <div className="flex-1 min-h-0 px-4 pb-0">
                  <TripMapPanel trip={selectedTrip} onFakeStart={pauseGps} onFakeStop={resumeGps} onStopSharing={stopSharing} />
                </div>
              </>
            )}
          </div>
