...
function TripInfoModal({ trip, onClose, onRemoveMember }) {
  ...
  const tabs = [
    ...
    { id: "members", label: `Thành viên (${memberUids.length})` },
  ];
          ...
          <div className="overflow-y-auto" style={{ height: "340px" }}>
            ...
            {activeTab === "info" && (
              <div className="px-6 py-4 flex flex-col gap-4">
                ...
                {/* Owner, Place */}
                {[
                  { label: "Chủ sở hữu", value: trip.owner?.display_name || trip.owner?.username || trip.owner_uid || "—" },
                  { label: "Địa điểm", value: trip.place?.name || trip.place_id || "—" },
                ].map((row) => (
                ...
                <div className="grid grid-cols-2 gap-3 pt-1 border-t border-gray-100">
                  <div className="flex flex-col gap-1">
                    <span className="text-[10px] font-semibold text-gray-400 uppercase tracking-wide">Bắt đầu lúc</span>
                    <span className="text-xs text-gray-700">{fmtDate(trip.dateFrom)}</span>
                  </div>
                  <div className="flex flex-col gap-1">
                    <span className="text-[10px] font-semibold text-gray-400 uppercase tracking-wide">Kết thúc lúc</span>
                ...
                <div className="grid grid-cols-2 gap-3 pt-1 border-t border-gray-100">
                  <div className="flex flex-col gap-1">
                    <span className="text-[10px] font-semibold text-gray-400 uppercase tracking-wide">Ngày tạo</span>
